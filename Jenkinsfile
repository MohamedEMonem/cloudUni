    ...
    stages{
        ...
        stage('build'){
            steps{
                ...
                // 2. Build the Frontend (with the relative API path injected)
                sh 'docker build --build-arg VITE_API_URL=/api -f ./Frontend/Dockerfile -t ${DOCKER_IMAGE_FRONT}:${BUILD_NUMBER} -t ${DOCKER_IMAGE_FRONT}:latest ./'
            }
        }
